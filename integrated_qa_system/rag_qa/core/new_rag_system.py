# Created by erainm on 2025/10/27 19:39.
# IDE：PyCharm 
# @Project: rag_system
# @File：new_rag_system
# @Description: 和之前的rag_system不一样的地方是：生成答案时，考虑了历史对话记录，以及我们大模型输出结果时stream流式输出结果
...
class RAGSystem:
    ...
    def generate_answer(self, query, source_filter=None, history=None):
        ...
        try:
            # 使用大模型获得输出结果：
            for token in self.llm(prompt_input,stream=True):
                yield token
